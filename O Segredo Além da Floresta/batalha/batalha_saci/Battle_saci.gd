...
func _ready():
	...
	esconde_atack()
	esconde_defesa()
	display_text("SACI O TRAVESSO TE DESAFIA")
...
func recarga_energia_aliado():
	if (energy_player <=max_energy):
		energy_player = max(0,energy_player + 5)
...
func enemy_turn():
				...
				if(aleatorio == 1):
					...
					if(energy_enemy<energy_atack):
						display_text("SACI ESTA SEM ESTAMINA")
						yield(self,"textbox_closed")
						...
						if rand_range(0, 99) > mira:
							display_text("SACI ERROU O BICUDA")
							...
							if rand_range(0, 99) < enemy_crit_chance:
								...
								set_health($PlayerData/Vida_Player,current_player_health,max_health)
								display_text("SACI atacou com BICUDA\n" + str(enemy_damage +5) +" Dano" )
								recarga_energia_inimigo()
								if(current_player_health==0):
				...
				if(aleatorio == 2):
					...
					if(energy_enemy<energy_atack):
						...
						mira = 45
						if rand_range(0, 99) > mira:
							display_text("SACI ERROU O ATAQUE\n Furacão")
							energy_enemy = max(0,energy_enemy - energy_atack)
							set_energy($EnemyData/Estamina,energy_enemy,enemy_energy)
							yield(self,"textbox_closed")
							recarga_energia_inimigo()
							...
							if rand_range(0, 99) < enemy_crit_chance:
								...
								current_player_health = max(0,current_player_health - dano_final)
								set_health($PlayerData/Vida_Player,current_player_health,max_health)
								display_text("SACI usou \nFuracão e DEU UM \n ATAQUE CRITICO \n" + str(int(dano_final)) +" Dano" )
								recarga_energia_inimigo()
								if(current_player_health==0):
									display_text("SACI te matou!")
									yield(self,"textbox_closed")
									get_tree().change_scene("res://cenas principais/DarkForest/DarkForest.tscn")
							else:
								energy_enemy = max(0,energy_enemy - energy_atack)
				...
				if(aleatorio == 3):
					...
					if(energy_enemy<energy_atack):
						...
						mira = 95
						if rand_range(0, 99) > mira:
							display_text("SACI ERROU O VOADORA \nDENTADA")
							energy_enemy = max(0,energy_enemy - energy_atack)
							set_energy($EnemyData/Estamina,energy_enemy,enemy_energy)
							yield(self,"textbox_closed")
							recarga_energia_inimigo()
							...
							if rand_range(0, 99) < enemy_crit_chance:
								...
								set_health($PlayerData/Vida_Player,current_player_health,max_health)
								display_text("SACI atacou com VOADORA\n" + str(enemy_damage) +" Dano" )
								recarga_energia_inimigo()
								if(current_player_health==0):
									display_text("Você MORREU!")
									yield(self,"textbox_closed")
			...
			if rand_range(0, 99) > mira:
				...
				if rand_range(0, 99) < crit_chance:
					...
					if(current_enemy_health==0):
							display_text("SACI DERROTADO")
							yield(self,"textbox_closed")
							get_tree().change_scene("res://batalha/batalha/batalha_boitata/Battle_boitata.tscn")
					else:
						enemy_turn()
						recarga_energia_aliado()
				else:
