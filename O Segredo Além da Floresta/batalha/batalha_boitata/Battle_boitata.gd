...
func _ready():
	...
	esconde_atack()
	esconde_defesa()
	display_text("BOITATA TE DESAFIA")
...
func enemy_turn():
				...
				if(aleatorio == 1):
					...
					if(energy_enemy<energy_atack):
						display_text(" BOITATA ESTA SEM ESTAMINA")
						yield(self,"textbox_closed")
						...
						if rand_range(0, 99) > mira:
							display_text(" BOITATA ERROU O RABADA")
							...
							if rand_range(0, 99) < enemy_crit_chance:
								...
								set_health($PlayerData/Vida_Player,current_player_health,max_health)
								display_text(" BOITATA atacou com RABADA\n" + str(enemy_damage +5) +" Dano" )
								recarga_energia_inimigo()
								if(current_player_health==0):
				...
				if(aleatorio == 2):
					...
					if(energy_enemy<energy_atack):
						...
						mira = 45
						if rand_range(0, 99) > mira:
							display_text(" BOITATA ERROU O ATAQUE\n LINGUADA")
							energy_enemy = max(0,energy_enemy - energy_atack)
							set_energy($EnemyData/Estamina,energy_enemy,enemy_energy)
							yield(self,"textbox_closed")
							recarga_energia_inimigo()
							...
							if rand_range(0, 99) < enemy_crit_chance:
								...
								set_health($PlayerData/Vida_Player,current_player_health,max_health)
								display_text(" BOITATA atacou com \nLINGUADA\n" + str(enemy_damage+8) +" Dano" )
								recarga_energia_inimigo()
								if(current_player_health==0):
									display_text("Você MORREU!")
									yield(self,"textbox_closed")
				...
				if(aleatorio == 3):
					...
					if(energy_enemy<energy_atack):
						...
						mira = 95
						if rand_range(0, 99) > mira:
							display_text(" BOITATA ERROU O PRESAS \nPRESAS")
							energy_enemy = max(0,energy_enemy - energy_atack)
							set_energy($EnemyData/Estamina,energy_enemy,enemy_energy)
							yield(self,"textbox_closed")
							recarga_energia_inimigo()
							...
							if rand_range(0, 99) < enemy_crit_chance:
								...
								set_health($PlayerData/Vida_Player,current_player_health,max_health)
								display_text(" BOITATA atacou com PRESAS\n" + str(enemy_damage) +" Dano" )
								recarga_energia_inimigo()
								if(current_player_health==0):
									display_text("Você MORREU!")
									yield(self,"textbox_closed")
			...
			if rand_range(0, 99) > mira:
				...
				if rand_range(0, 99) < crit_chance:
					...
					if(current_enemy_health==0):
							...
							yield(self,"textbox_closed")
							get_tree().change_scene("res://batalha/batalha/batalha_boitata_final/Battle_boitata_final.tscn")
					else:
						enemy_turn()
						recarga_energia_aliado()
				else:
