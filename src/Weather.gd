...
export (String, 'clear', 'rain', 'snow', 'windy') var weatherType = 'sun'
...
export (float, 0, 1) var windy_darkness = 0.05
...
func change_weather():
	...
	if weatherType == 'windy':
		...
		# DARKEN DAY
		change_light(nightColor.darkened(light - windy_darkness * size))
		if delayWeatherChange: yield(tween, "tween_completed") # Waits light change to change weather
		
		change_amount(windy, amount)
		apply_snow_settings()
		windy.emitting = true
		
	else: windy.emitting = false
...
func apply_windy_settings():
	
	# WINDY SETTINGS
	change_size(windy, size) # snow.process_material.anim_offset = size
	
	# WINDY WIND SETTINGS
	change_wind_speed(windy, 0.5 + abs(wind) / 2) # snow.speed_scale = 0.5 + abs(wind) / 2
	change_wind_direction(windy, wind) # snow.process_material.direction.x = wind
	windy.process_material.gravity.x = 70 * wind
