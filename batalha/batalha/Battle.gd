...
func _ready():
	...
	esconde_opcoes()
	esconde_atack()
	display_text(enemy.name+" te desafiou para o Duelo")
	yield(self,"textbox_closed")
	mostra_opcoes()
	set_health($EnemyData/VidaInimigo , enemy.health , enemy.health)
	set_health($PlayerData/Vida_Player , current_health, max_health)
	set_energy($EnemyData/Estamina , enemy.energy,enemy.max_energy)
	set_energy($PlayerData/Estamina , energy,max_energy)
	current_player_health = current_health
	current_enemy_health = enemy.health
	energy_enemy = enemy.energy
...
func recarga_energia_inimigo():
	...
	set_energy($EnemyData/Estamina,energy_enemy,enemy.energy)
...
func enemy_turn():
	var aleatorio = int(rand_range(1,4))
	if(aleatorio == 1):
		energy_atack = 50
		if(energy_enemy<energy_atack):
			display_text("SEU INIMIGO ESTA SEM ESTAMINA")
			yield(self,"textbox_closed")
			...
			if rand_range(0, 99) > mira:
				display_text("SEU INIMIGO ERROU O ATAQUE\n SACOLADA")
				energy_enemy = max(0,energy_enemy - energy_atack)
				set_energy($EnemyData/Estamina,energy_enemy,enemy.energy)
				yield(self,"textbox_closed")
				recarga_energia_inimigo()
			else:
				if rand_range(0, 99) < enemy.crit_chance:
					energy_enemy = max(0,energy_enemy - energy_atack)
					set_energy($EnemyData/Estamina,energy_enemy,enemy.energy)
					multiplicador = rand_range(enemy.crit_min, enemy.crit_max)
					var dano_final = (enemy.damage * multiplicador) +20
					...
					energy_enemy = max(0,energy_enemy - energy_atack)
					set_energy($EnemyData/Estamina,energy_enemy,enemy.energy)
					current_player_health = max(0,current_player_health - enemy.damage - 20)
					set_health($PlayerData/Vida_Player,current_player_health,max_health)
					display_text(enemy.name+" atacou com SACOLADA\n" + str(enemy.damage +20) +" Dano" )
					recarga_energia_inimigo()
					if(current_player_health==0):
		...
		if(energy_enemy<energy_atack):
			...
			if rand_range(0, 99) > mira:
				...
				if rand_range(0, 99) < enemy.crit_chance:
					...
					set_energy($EnemyData/Estamina,energy_enemy,enemy.energy)
					multiplicador = rand_range(enemy.crit_min, enemy.crit_max)
					var dano_final = (enemy.damage * multiplicador)+8
					current_player_health = max(0,current_player_health - dano_final)
					set_health($PlayerData/Vida_Player,current_player_health,max_health)
					display_text("Seu inimigo usou \nBAFO DE ONÇA e DEU UM \n ATAQUE CRITICO \n" + str(int(dano_final)) +" Dano" )
					recarga_energia_inimigo()
					if(current_player_health==0):
						display_text("Seu inimigo te matou!")
						yield(self,"textbox_closed")
					...
					energy_enemy = max(0,energy_enemy - energy_atack)
					set_energy($EnemyData/Estamina,energy_enemy,enemy.energy)
					current_player_health = max(0,current_player_health - enemy.damage-8)
					set_health($PlayerData/Vida_Player,current_player_health,max_health)
					display_text(enemy.name+" atacou com \nBAFO DE ONÇA\n" + str(enemy.damage+8) +" Dano" )
					recarga_energia_inimigo()
					if(current_player_health==0):
						display_text("Você MORREU!")
						yield(self,"textbox_closed")
		...
		if(energy_enemy<energy_atack):
			...
			if rand_range(0, 99) > mira:
				...
				if rand_range(0, 99) < enemy.crit_chance:
					...
					set_energy($EnemyData/Estamina,energy_enemy,enemy.energy)
					multiplicador = rand_range(enemy.crit_min, enemy.crit_max)
					var dano_final = (enemy.damage * multiplicador) 
					current_player_health = max(0,current_player_health - dano_final)
					set_health($PlayerData/Vida_Player,current_player_health,max_health)
					display_text("Seu inimigo usou\n DOCE\n e DEU UM \n ATAQUE CRITICO \n" + str(int(dano_final + 20)) +" Dano" )
					recarga_energia_inimigo()
					...
					set_health($PlayerData/Vida_Player,current_player_health,max_health)
					display_text(enemy.name+" atacou com DOCE\n" + str(enemy.damage) +" Dano" )
					recarga_energia_inimigo()
					if(current_player_health==0):
						display_text("Você MORREU!")
						yield(self,"textbox_closed")
			...
			if(energy_enemy<energy_atack):
				...
				if rand_range(0, 99) > mira:
					...
					if rand_range(0, 99) < enemy.crit_chance:
						...
						set_energy($EnemyData/Estamina,energy_enemy,enemy.energy)
						multiplicador = rand_range(enemy.crit_min, enemy.crit_max)
						var dano_final = (enemy.damage * multiplicador) +5
						current_player_health = max(0,current_player_health - dano_final)
						set_health($PlayerData/Vida_Player,current_player_health,max_health)
						display_text("Seu inimigo usou\n BARATADA\n e DEU UM \n ATAQUE CRITICO \n" + str(int(dano_final + 20)) +" Dano" )
						recarga_energia_inimigo()
						...
						energy_enemy = max(0,energy_enemy - energy_atack)
						set_energy($EnemyData/Estamina,energy_enemy,enemy.energy)
						current_player_health = max(0,current_player_health - enemy.damage-5)
						set_health($PlayerData/Vida_Player,current_player_health,max_health)
						display_text(enemy.name+" atacou com BARATADA\n" + str(enemy.damage+5) +" Dano")
						recarga_energia_inimigo()
						if(current_player_health==0):
							display_text("Você MORREU!")
							yield(self,"textbox_closed")
	...
	if(energy_player<energy_atack):
		...
		if rand_range(0, 99) > mira:
			...
			if rand_range(0, 99) < crit_chance:
				...
				var dano_final = (damage * multiplicador) + 20
				current_enemy_health = max(0,current_enemy_health - dano_final)
				set_health($EnemyData/VidaInimigo,current_enemy_health,enemy.health)
				display_text("Você atacou e Deu um \n ATAQUE CRITICO \n" + str(int(dano_final + 20)) +" Dano" )
				yield(self,"textbox_closed")
				if(current_enemy_health==0):
					display_text("VOCE MORREU")
					yield(self,"textbox_closed")
				...
				recarga_energia_aliado()
				if(current_enemy_health==0):
					display_text(enemy.name +" MORREU")
					yield(self,"textbox_closed")
					get_tree().change_scene("res://Pântano.tscn")
				else:
					enemy_turn()
					recarga_energia_aliado()
